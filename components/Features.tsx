...
const Features: React.FC = () => {
  return (
    <section id="features" className="relative py-32 px-6 w-full max-w-7xl mx-auto">
      ...
      <div className="mb-20">
        <h2 className="text-5xl md:text-7xl font-medium text-slate-900 mb-6 leading-[0.95] tracking-tight max-w-3xl">
          Everything you need. <br/>
          <span className="font-serif italic text-slate-500">Nothing you don't.</span>
        </h2>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 auto-rows-[minmax(280px,auto)]">
        
        {/* Card 1: Analytics (Vibrant Gradient) */}
        <motion.div 
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h3 className="text-3xl font-medium mb-2">Smart Analytics</h3>
            <p className="text-white/80 max-w-sm font-medium text-lg">
              Visualize your spending habits with beautiful, auto-generated charts.
            </p>
          </div>
        ...
        >
          ...
          <div>
            <h3 className="text-2xl font-medium mb-2">Instant Sync</h3>
            <p className="text-slate-400 text-sm leading-relaxed">Connects with 5,000+ banks instantly.</p>
          </div>
        </motion.div>

        {/* Card 3: Security (Light) */}
        <motion.div 
          whileHover={{ scale: 1.02 }}
          className="md:col-span-1 bg-slate-100 rounded-[2.5rem] p-8 flex flex-col justify-between hover:bg-white hover:shadow-xl transition-all duration-300"
        >
          <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-slate-900 mb-4 shadow-sm">
            <Shield className="w-6 h-6" />
          </div>
          <div>
            <h3 className="text-2xl font-medium text-slate-900 mb-2">Secure</h3>
            <p className="text-slate-500 text-sm leading-relaxed">Bank-grade encryption standards.</p>
          </div>
        </motion.div>

        {/* Card 4: Virtual Cards (Big Graphic) */}
        <motion.div 
          whileHover={{ scale: 1.02 }}
        ...
        >
           <div className="relative z-10 flex flex-col h-full justify-between">
              <div>
                ...
                <h3 className="text-3xl font-medium text-slate-900 mb-2">Virtual Cards</h3>
                <p className="text-slate-500 text-lg max-w-xs">
                  Create disposable cards for safe online shopping.
                </p>
              </div>

              {/* Abstract Card Graphic */}
              <div className="absolute -bottom-12 -right-12 w-64 h-40 bg-blue-600 rounded-2xl shadow-2xl transform -rotate-12 group-hover:-rotate-6 transition-all duration-500 flex flex-col justify-between p-5 border-t border-white/20">
                 <div className="flex justify-between">
                   <div className="w-8 h-5 bg-white/20 rounded"></div>
                   <div className="text-white/50 font-mono text-xs">DEBIT</div>
                 </div>
                 <div className="text-white/80 font-mono tracking-widest text-sm">**** 4242</div>
              </div>
           </div>
        </motion.div>

        {/* Card 5: Notifications (Purple) */}
        <motion.div 
          whileHover={{ scale: 1.02 }}
        ...
        >
          ...
          <div>
             ...
             <p className="text-white/80 text-sm">Get notified before you overspend.</p>
          </div>
        </motion.div>

        {/* Card 6: Call to Action Card (Yellow) */}
        <motion.div 
          whileHover={{ scale: 0.98 }}
          className="md:col-span-1 bg-yellow-400 rounded-[2.5rem] p-8 shadow-xl flex flex-col justify-center items-center text-center gap-4 cursor-pointer group"
        >
          ...
          <h3 className="text-2xl font-medium text-slate-900 leading-tight font-serif italic">Start your journey</h3>
        </motion.div>
